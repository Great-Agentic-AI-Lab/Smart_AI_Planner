...
"""
from sqlalchemy import Column, Integer, String, Boolean, JSON, ForeignKey, Time, Date
...
class UserPreferences(Base):
    ...
    - Analytics opt-in
    - Last sent tracking (prevents duplicate messages)
    """
    ...
    # ✅ NEW: Last sent tracking - prevents spam (one message per day per type)
    last_birthday_sent_date = Column(Date, nullable=True)    # Date last birthday wish was sent
    last_festival_sent_date = Column(Date, nullable=True)    # Date last festival greeting was sent
    last_digest_sent_date = Column(Date, nullable=True)      # Date last daily digest was sent
